"use server";

import { revalidatePath, revalidateTag } from "next/cache";
import { requireAdmin } from "@/lib/auth";
import { dbConnect } from "@/lib/db/dbConnect";
import { SpecificationDefinition } from "@/models/Specification";
import { actionSuccess, actionError, type ActionResult } from "@/lib/action-result";
import { updateSpecificationSchema } from "../validation";

export async function updateSpecification(formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
    await requireAdmin();
  } catch {
    return actionError("Admin access required");
  }

  const id = formData.get("id")?.toString() ?? "";
  if (!id) return actionError("Specification id is required");

  const raw = {
    id,
    name: formData.get("name")?.toString() ?? "",
    categoryId: formData.get("categoryId")?.toString() ?? "",
    values: formData.get("values")?.toString() ?? "",
    status: formData.get("status")?.toString() ?? "active",
  };

  const parsed = updateSpecificationSchema.safeParse(raw);
  if (!parsed.success) {
    return actionError("Please fix the errors below", parsed.error.flatten().fieldErrors);
  }
  const { id: _id, ...data } = parsed.data;

  await dbConnect();

  const existing = await SpecificationDefinition.findById(id);
  if (!existing) return actionError("Specification not found");

  existing.set(data);

  try {
    await existing.save();
    revalidatePath("/admin/specifications");
    revalidateTag("specifications", "max");
    return actionSuccess({ id }, "Specification updated");
  } catch (error: unknown) {
    if (typeof error === "object" && error !== null && "code" in error && error.code === 11000) {
      return actionError("Please fix the errors below", {
        name: ["A specification with this name already exists for this category"],
      });
    }
    console.error("Failed to update specification:", error);
    return actionError("Failed to update specification. Please try again.");
  }
}