...
if (!signingSecret) {
  console.warn(
    "CLERK_WEBHOOK_SIGNING_SECRET / CLERK_WEBHOOK_SECRET is not set. Clerk webhook verification will fail at runtime until it is configured."
  );
}
...
export const env = {
  ...
  CLERK_WEBHOOK_SECRET: process.env.CLERK_WEBHOOK_SECRET ?? process.env.CLERK_WEBHOOK_SIGNING_SECRET ?? "",
